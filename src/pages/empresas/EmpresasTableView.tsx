...
function renderReceitaBadges(e: Empresa) {
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {sit && (
        <Badge variant="secondary" className={`text-[10px] ${sitColor}`}>
          {sit}
        </Badge>
      )}
    ...
    <Card className="overflow-hidden shadow-card">
                ...
                >
                  ...
                  <td className="px-3 py-2.5 font-medium">
                    <button
                      ...
                      className="text-left hover:text-primary hover:underline focus:underline focus:outline-none"
                    >
                  ...
                  <td className="px-3 py-2.5">
                    <span
                      className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${statusColors[e.status] || ""}`}
                    >
                      {e.status}
                    </span>
                  </td>
                  ...
                  <td className="whitespace-nowrap px-3 py-2.5 text-xs">
                    {(() => {
                      ...
                      if (!ff.hasAny) return <span className="text-muted-foreground">—</span>;
                      return (
                        <div className="flex flex-col gap-0.5 leading-tight">
                          <span>{ff.faturamento ?? "—"}</span>
                          <span className="text-muted-foreground">{ff.funcionarios ?? "—"}</span>
                        </div>
                      );
                    })()}
                  </td>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border px-4 py-3">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span>
            de <span className="font-medium tabular-nums text-foreground">{total}</span> empresa
            {total === 1 ? "" : "s"}
          </span>
        </div>
