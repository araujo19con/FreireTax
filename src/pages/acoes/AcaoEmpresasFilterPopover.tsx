  ...
  filters.statusCombinado?.forEach((v) => {
    ...
    chips.push({
      ...
      onRemove: () =>
        onChange({
          ...filters,
          statusCombinado: (filters.statusCombinado || []).filter((x) => x !== v),
        }),
    });
  });
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        onChange({
                          ...filters,
                          statusCombinado: toggleArrayValue<StatusCombinadoKey>(
                            filters.statusCombinado,
                            opt.key
                          ),
                        })
                      }
                      className={`h-7 truncate rounded-md border px-2 text-[11px] transition-colors ${
                        active
                ...
                {UF_OPTIONS.map((uf) => {
                  ...
                  return (
                    <button
                      key={uf}
                      type="button"
                      onClick={() => onChange({ ...filters, uf: toggleArrayValue(filters.uf, uf) })}
