    ...
    const load = async () => {
      ...
      setRows(
        ((data as CicloRow[]) ?? []).map((r) => ({
          ...
          dias_medios_na_etapa:
            r.dias_medios_na_etapa == null ? null : Number(r.dias_medios_na_etapa),
        }))
      );
      setLoading(false);
    };
    ...
    <Card className="p-6 shadow-card">
      ...
      {semDados ? (
        <p className="text-xs text-muted-foreground">
          Ainda sem transições registradas. Conforme os negócios avançam de etapa, o tempo médio
          aparece aqui.
        </p>
      ) : (
        <div className="space-y-3">
